...
vi.mock("../../agents/pi-embedded-runner/runs.js", async () => {
  ...
  return {
    ...actual,
    queueEmbeddedPiMessageWithOutcome: (...args: unknown[]) =>
      queueEmbeddedPiMessageWithOutcomeMock(...args),
  };
});
...
beforeEach(() => {
  ...
  queueEmbeddedPiMessageWithOutcomeMock.mockClear();
...
describe("runReplyAgent steer mode message injection", () => {
  function createSteerRun(params: {
    ...
    queueEmbeddedResult?: { queued: boolean };
  }) {
    ...
    queueEmbeddedPiMessageWithOutcomeMock.mockReturnValue(
      params.queueEmbeddedResult ?? {
        queued: false,
        sessionId: "session-steer",
  ...
  it("injects steer message when shouldSteer=true AND isActive=true", async () => {
    const { promise, typing } = createSteerRun({
      shouldSteer: true,
      isActive: true,
      queueEmbeddedResult: { queued: true },
    });
  ...
  it("does NOT inject steer message when shouldSteer=true AND isActive=false", async () => {
    ...
    expect(queueEmbeddedPiMessageWithOutcomeMock).not.toHaveBeenCalled();
